...
while(__HAL_UART_GET_FLAG(&huart1,UART_FLAG_TC)!=SET);//根据想定向到哪个串口决定
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_Base_Start_IT(&htim12);//开启定时器12
  /* USER CODE END 2 */
  ...
  while (1)
  {
		// SR_04_Start(GPIOA, GPIO_PIN_2);
		// HAL_Delay(10);
    // printf("distance = %f\r\n", SR_04.distance);
    // printf("distance_filtered = %f\r\n", SR_04.distance_filtered);
		//printf("joker\r\n");
